  ...
  try {
    const response = await fetch(url.toString(), {
      ...
      body: JSON.stringify({
        ...
        instructions: true,
        geometry: true,
        // Preference 'fastest' helps avoid some weird low-speed slip-road shortcuts
        preference: 'fastest' 
      })
    });
...
function normalizeStepType(type) {
  const map = { 
    ...
    12: 'turn-left', 13: 'turn-right', 11: 'depart', 10: 'arrive' 
  };
...
export function transformStepsToTurns(steps) {
  ...
  // 1. ADVANCED MERGING: Ignore \"noise\" at intersections
  const cleanSteps = [];
  for (let i = 0; i < steps.length; i++) {
    let current = { ...steps[i] };
    
    // If this step is very short (<100m) and the next step is also a turn, 
    // merge them to avoid \"turn left then immediately turn right\" noise at junctions.
    if (i < steps.length - 1 && current.distance < 100) {
      const next = steps[i + 1];
      if (next.type !== 'straight' && next.type !== 'arrive') {
        next.distance += current.distance;
        continue; // Skip the current \"noise\" step
      }
    }
  ...
  turns.push({
    instruction: `Drive onto ${first.name || 'route'}`,
