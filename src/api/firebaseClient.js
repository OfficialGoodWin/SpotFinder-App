...
// ─── POI Photos (SpotFinder-exclusive, keyed by OSM poiId) ───────────────────
const POI_PHOTOS_COLLECTION = 'poi_photos';
const POI_RATINGS_COLLECTION = 'poi_ratings';

export const getPOIPhotos = async (poiId) => {
  const { db } = getFirebaseServices();
  const q = query(collection(db, POI_PHOTOS_COLLECTION), where('poiId', '==', poiId), orderBy('createdAt', 'desc'), limit(20));
  return (await getDocs(q)).docs.map(d => ({ id: d.id, ...d.data() }));
};

export const addPOIPhoto = async (poiId, url, addedBy) => {
  const { db } = getFirebaseServices();
  return await addDoc(collection(db, POI_PHOTOS_COLLECTION), {
    poiId, url, addedBy, createdAt: new Date().toISOString()
  });
};

// ─── POI Ratings (SpotFinder-exclusive) ──────────────────────────────────────
export const getPOIRatings = async (poiId) => {
  const { db } = getFirebaseServices();
  const q = query(collection(db, POI_RATINGS_COLLECTION), where('poiId', '==', poiId), orderBy('createdAt', 'desc'), limit(100));
  return (await getDocs(q)).docs.map(d => ({ id: d.id, ...d.data() }));
};

export const addPOIRating = async (poiId, rating, addedBy) => {
  const { db } = getFirebaseServices();
  return await addDoc(collection(db, POI_RATINGS_COLLECTION), {
    poiId, rating, addedBy, createdAt: new Date().toISOString()
  });
};

// ─── POI Photos & Ratings (SpotFinder-exclusive, keyed by OSM element ID) ────

const POI_PHOTOS_COLLECTION  = 'poi_photos';
const POI_RATINGS_COLLECTION = 'poi_ratings';

/** Stable key: "osm_<osmId>" */
export const poiKey = (osmId) => `osm_${osmId}`;

/** Add a photo (base64 data URL) to a POI */
export const addPOIPhoto = async (osmId, photoDataUrl, userEmail) => {
  const { db } = getFirebaseServices();
  return addDoc(collection(db, POI_PHOTOS_COLLECTION), {
    poi_id: poiKey(osmId),
    photo: photoDataUrl,
    created_by: userEmail || 'anonymous',
    created_date: new Date().toISOString(),
...
/** Get all SpotFinder photos for a POI */
export const getPOIPhotos = async (osmId) => {
  const { db } = getFirebaseServices();
  const q = query(
    collection(db, POI_PHOTOS_COLLECTION),
    where('poi_id', '==', poiKey(osmId)),
    orderBy('created_date', 'desc'),
    limit(20)
  );
  return (await getDocs(q)).docs.map(d => ({ id: d.id, ...d.data() }));
...
/** Submit a 1–5 star rating + optional comment for a POI */
export const addPOIRating = async (osmId, rating, comment, userEmail) => {
  const { db } = getFirebaseServices();
  return addDoc(collection(db, POI_RATINGS_COLLECTION), {
    poi_id: poiKey(osmId),
    rating,
    comment: comment || '',
    created_by: userEmail || 'anonymous',
...
/** Get SpotFinder ratings for a POI, returns { ratings[], avg, count } */
export const getPOIRatings = async (osmId) => {
  const { db } = getFirebaseServices();
  const q = query(
    collection(db, POI_RATINGS_COLLECTION),
    where('poi_id', '==', poiKey(osmId)),
  ...
  const avg = docs.length
    ? Math.round((docs.reduce((s, d) => s + d.rating, 0) / docs.length) * 10) / 10
    : 0;
  return { ratings: docs, avg, count: docs.length };
};