...
import { LogOut, List, Trash2, User, LogIn } from 'lucide-react';

export default function ProfileMenu({
  user, isAuthenticated,
  showMenu, onToggleMenu,
  onShowMySpots, onSignOut, onShowDeleteConfirm, onShowAuth
}) {
  return (
    <div className="absolute top-4 right-4 z-[1003]">
      ...
      {isAuthenticated && user ? (
        <>
          ...
          {showMenu && (
            <div className="absolute top-full right-0 mt-2 w-44 bg-white dark:bg-card rounded-2xl shadow-2xl border border-gray-100 dark:border-border py-1.5 overflow-hidden">
              ...
              <button
                onClick={() => { onShowMySpots(); onToggleMenu(); }}
              ...
              >
                <List className="w-4 h-4" /> My Spots
              </button>
              <button
                onClick={() => { onSignOut(); onToggleMenu(); }}
                className="w-full px-4 py-2.5 text-left text-sm text-gray-700 dark:text-foreground hover:bg-gray-50 dark:hover:bg-accent flex items-center gap-2.5 transition-colors"
              >
              ...
              <div className="border-t border-gray-100 dark:border-border mt-1 pt-1">
                <button
                  onClick={() => { onShowDeleteConfirm(); onToggleMenu(); }}
