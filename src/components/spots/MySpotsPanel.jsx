...
export default function MySpotsPanel({ user, onClose, onFlyTo }) {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-end justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white w-full max-w-lg rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col">
        <div className="px-6 pt-5 pb-3 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
          <h2 className="text-lg font-bold text-gray-900">My Spots</h2>
        ...
        <div className="overflow-y-auto flex-1 px-4 py-3 space-y-3">
          ...
          {!loading && spots.length === 0 && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-400 font-medium">No spots yet</p>
              <p className="text-gray-300 text-sm">Add your first spot on the map</p>
            </div>
          )}
          {spots.map(spot => {
            ...
            const { Icon } = cfg;
            return (
              <div key={spot.id} className="bg-gray-50 rounded-2xl p-4 flex items-start gap-3">
                <div className={`w-10 h-10 rounded-xl ${cfg.bg} flex items-center justify-center flex-shrink-0`}>
                  <Icon className={`w-5 h-5 ${cfg.color}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-800 truncate">{spot.title || cfg.label}</p>
                  {spot.description && <p className="text-xs text-gray-500 truncate mt-0.5">{spot.description}</p>}
                  <div className="mt-1">
