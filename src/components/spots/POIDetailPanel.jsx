...
//
// COMPLIANCE NOTE (flagged, not fixed here): neither remaining source's
// attribution is currently shown to the user.
...
//    Google Photos.
//  - Wikimedia Commons images (both the OSM-tag and Wikidata tiers) are
//    almost always CC-BY-SA or similar, which legally requires attribution
//    (author + license) wherever the image is used. Consider fetching
//    `imageinfo` with `iiprop=url|extmetadata` and showing
//    "Photo: <author>, via Wikimedia Commons (<license>)" near the image,
//    or a small credit line in the lightbox.
...
async function fetchWikidataPhoto(wikidataId) {
  ...
  try {
    ...
    return claims
      ...
      .filter(Boolean)
      .map(file => `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(file)}?width=1200`);
  } catch { return []; }
}
...
export default function POIDetailPanel({ poi, category, onClose, onNavigate, user }) {
  ...
  useEffect(() => {
    ...
    getPOIPhotos(poiId)
      .then(setSfPhotos)
      .catch(err => console.error('[POIDetailPanel] Failed to load community photos:', err));
    getPOIRatings(poiId)
      .then(r => setSfRating(
        Array.isArray(r)
      ...
      .catch(err => console.error('[POIDetailPanel] Failed to load ratings:', err));
    tryFetchPhotos(poi.name, poi.lat, poi.lon, poi.tags || {})
      .then(urls => { if (urls.length) setPhotos(urls); })
      .catch(err => console.error('[POIDetailPanel] Failed to fetch external photos:', err));
  }, [poi?.id]);
