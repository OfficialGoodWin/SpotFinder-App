/**
 * MapLibreMap.jsx  (updated for OPFS + high-zoom tile cache)
 *
 * Tile serving strategy:
 *   Online:   OpenFreeMap vector tiles via PMTiles range requests
 *   Offline z0-14:  Country .pmtiles file read from OPFS (downloaded via OfflineMapsMenu)
 *   Offline z15-19: Per-tile IndexedDB cache (populated as user browses online)
 *
 * All other logic (road shields, E-routes, spot markers, POIs, routing overlay,
 * traffic, aerial layers) is unchanged from the original.
 */
...
import { COUNTRIES, isPointInCountry, getDownloadedCountryAt, pmtilesFilename } from '../../lib/vectorTileDownloader.js';
...
// Online tile server — falls back to OpenFreeMap if VITE_TILE_SERVER not set
...
/**
 * Registers three MapLibre protocols:
 *
 *  pmtiles://     — default PMTiles streaming (unchanged)
 *  offline-base:// — serves z0-14 from the country .pmtiles file in OPFS
 *  offline-hz://  — serves z15-19 from the IndexedDB high-zoom tile cache
 *                    AND caches tiles fetched online for future offline use
 */
async function ensureProtocols() {
  if (protocolsRegistered) return;

  // 1. Standard PMTiles range-request protocol
...
// ── OPFS PMTiles instance cache ───────────────────────────────────────────────
// Keep PMTiles instances open — re-opening a File is cheap but the index parse isn't.

const _pmtilesInstances = new Map();

async function getOrOpenPMTiles(code) {
  if (_pmtilesInstances.has(code)) return _pmtilesInstances.get(code);
  const filename = pmtilesFilename(code);
  const p = await openFromOPFS(filename);
  _pmtilesInstances.set(code, p);
  return p;
}

// ── Offline country detection ─────────────────────────────────────────────────

let _activeCountryCache     = null;
let _activeCountryCacheTime = 0;

async function getActiveOfflineCountry(lat, lng) {
  if (Date.now() - _activeCountryCacheTime < 5000) return _activeCountryCache;

  const meta = await getAllMeta();
  for (const [code, m] of Object.entries(meta)) {
    if (m?.type !== 'pmtiles') continue;
    const filename = pmtilesFilename(code);
    if (!(await hasFile(filename))) continue;
    _activeCountryCache     = COUNTRIES.find(c => c.code === code) || null;
    _activeCountryCacheTime = Date.now();
    return _activeCountryCache;
  }
  _activeCountryCache     = null;
  _activeCountryCacheTime = Date.now();
  return null;
}

// ── Ambient POI categories (unchanged) ───────────────────────────────────────
const AMBIENT_CATS = [
...
function makeDot(emoji, color, size = 28) {
  ...
  el.style.cssText = `width:${size}px;height:${size}px;border-radius:50%;background:${color};border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.35);display:flex;align-items:center;justify-content:center;font-size:${Math.round(size * 0.5)}px;line-height:1;cursor:pointer;user-select:none;`;
...
function makeSpotDom(type) {
  ...
  el.innerHTML = `<div style="width:36px;height:36px;background:${c};border-radius:50% 50% 50% 0;transform:rotate(-45deg);display:flex;align-items:center;justify-content:center;box-shadow:0 2px 6px rgba(0,0,0,0.35);border:2px solid white;"><span style="transform:rotate(45deg);font-size:16px;line-height:1">${i}</span></div>`;
  return el;
}
...
// ── Build the offline style (OPFS base + high-zoom cache) ────────────────────

function buildOfflineStyle(baseStyle, country) {
  // z0-14 tiles come from the OPFS PMTiles file via offline-base://
  // z15-19 tiles come from the IndexedDB tile cache via offline-hz://
  return {
    ...baseStyle,
    sources: {
      v: {
...
export default function MapLibreMap({
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      zoom:               13,
      minZoom:            3,
      maxZoom:            19, // Allow zoom 19 now that high-zoom cache supports it
    ...
    // Road shield generator (unchanged — fires on styleimagemissing)
    map.on('styleimagemissing', (e) => {
      if (e.id?.startsWith('shield-')) addShieldImage?.(map, e.id);
    });
  ...
  useEffect(() => {
    ...
    async function checkOffline() {
      ...
      const country = await getActiveOfflineCountry();
      if (country) {
        const offlineStyle = buildOfflineStyle(isDark ? darkStyle : lightStyle, country);
        map.setStyle(offlineStyle);
        setOfflineActive(true);
        setOfflineCountry(country.name);
      }
    }
    ...
    <div style={{ width:'100%', height:'100%', position:'relative' }}>
      ...
      {!isOnline && (
        <div style={{ position:'absolute', top:8, right:8, zIndex:10, pointerEvents:'none' }}>
          <span style={{ fontSize:11, padding:'3px 8px', borderRadius:12, background: offlineActive ? '#2563eb' : '#6b7280',
            color:'#fff', fontWeight:600, boxShadow:'0 2px 8px rgba(0,0,0,0.3)', opacity:0.9 }}>
            {offlineActive ? `📴 Offline · ${offlineCountry}` : '📴 Offline · No map downloaded'}
          </span>
        </div>
      )}
    </div>
